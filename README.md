# SQL Path — Interactive course (`sql-playground.html`)
...
**Main file:** [`sql-playground.html`](sql-playground.html)

## How to open it

1. Double-click `sql-playground.html`, or open it from your editor’s “Open in browser” action.
...
Download `sql-wasm.js` and `sql-wasm.wasm` from the [sql.js releases](https://github.com/sql-js/sql.js/releases) (match version **1.11.0** with the URLs in the file), place them next to `sql-playground.html`, and change the `<script src="…">` and `locateFile` URL in the HTML to relative paths.
...
[`splice_curriculum.py`](splice_curriculum.py) is a small helper that can replace the `/* ——— Curriculum ——— */` block in `sql-playground.html` with the contents of a separate text file. The authoritative curriculum in this repo is **inside** `sql-playground.html`; the script is optional for bulk edits.
